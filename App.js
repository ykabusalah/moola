...
// Support link - change this to your actual link
const SUPPORT_LINK = 'https://ko-fi.com/ykabusalah'; // Replace with your link
const ACCENT_COLORS = [
...
export default function App() {
  ...
  const [settingsPage, setSettingsPage] = useState('main'); // 'main', 'currency', 'accent', 'privacy', 'about'
  ...
  // Override soul colors with accent
  ...
  // Format amount based on preferences
  ...
  // Format display for large numbers (split whole and decimal)
  ...
  const savePreferences = async () => {
    try {
      await AsyncStorage.setItem(STORAGE_KEYS.PREFERENCES, JSON.stringify({ 
        name, dark, startDate, onboardingComplete, currency: currency.code,
        accentColor: accentColor.id, useEUFormat, hideDecimals
      }));
    } catch (error) {
  ...
  const exportShare = async () => {
    try {
      ...
      if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(filepath, { mimeType: 'text/csv', dialogTitle: 'Export Expenses' });
      setShowExport(false);
  ...
  const exportSave = async () => {
    try {
      ...
      const filepath = FileSystem.documentDirectory + `moola-export-${formatDate(getToday())}.csv`;
      await FileSystem.writeAsStringAsync(filepath, csv, { encoding: 'utf8' });
      if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(filepath, { mimeType: 'text/csv', dialogTitle: 'Save Expenses', UTI: 'public.comma-separated-values-text' });
      setShowExport(false);
    ...
    <View style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          {/* Time Progress - Different style per period */}
          <View style={{ paddingHorizontal: 24, paddingBottom: 20 }}>
            
            {/* TODAY: Flowing wave with gradient-like segments */}
            {period === 'today' && (
              <View>
